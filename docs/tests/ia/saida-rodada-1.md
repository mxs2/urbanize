...
Saída íntegra, sem correção. Prompt registrado no plano de testes (seção 4.1).
A IA gerou duas partes: a resposta no chat e o arquivo `docs/tests/casos-de-teste.md`.
As duas estão copiadas abaixo sem alteração.

---

## Parte 1 — Resposta no chat

Escrevi 115 casos de teste em docs/tests/casos-de-teste.md. Eles partem do código atual (backend, telas mobile e seed), não só da descrição. Cada caso traz ID, pré-condição, passos, resultado esperado, camada (API, UI ou manual) e prioridade de P0 a P2, no mesmo padrão do plano-de-testes.md.

Módulo	Casos
Autenticação	16
Controle de acesso por perfil	10
Foto e upload	13
Classificação por imagem (Vision e fallback)	13
Formulário de nova demanda	19
Localização	6
Acompanhamento pelo cidadão	9
Painel e triagem do gestor	12
Alteração de status	11
Não funcionais	6
Na leitura do código apareceram cinco prováveis defeitos. Eles estão marcados com ⚠ no documento e vêm antes da automação:

Crítico (CT-AUTH-10): qualquer pessoa pode se cadastrar como gestor. POST /api/auth/register aceita role: "gestor" e a tela de cadastro mostra o campo "Perfil". Quem fizer isso pode alterar o status de todas as demandas.
CT-VIS-12: se o Google Vision falhar, o upload retorna erro 500 em vez de usar a categoria alternativa, e a imagem fica largada no disco.
CT-DEM-12: a demanda grava confiança fixa de 82% (55% para "outros") e ignora a nota que a análise da imagem deu de fato.
CT-FOTO-11: o upload confia no tipo de arquivo declarado pelo aparelho, então um arquivo que não é imagem passa se vier marcado como PNG.
CT-STA-08 e 09: qualquer mudança de status é aceita. Uma demanda resolvida pode voltar para "em análise", e repetir o mesmo status duplica a entrada no histórico. Falta vocês definirem a regra de negócio para isso.
Nenhum caso foi executado ainda. Posso automatizar os P0 de API com Jest + Supertest em tests/acceptance/, se quiser.

---

## Parte 2 — Arquivo gerado (`docs/tests/casos-de-teste.md`, cópia literal)
